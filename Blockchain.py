...
class Blockchain(object):
    ...
    def valid_chain(self, chain):
        ...
        # correct by hashing the previous block and then comparing it with the one recorded in the current block
...
node_identifier = str(uuid4()).replace('-',"508")
...
def mine_block():
    # the miner must receive a reward for finding the proof the sender is "0" to signify that this node has mined a 
    #new coin.
...
if __name__ == '__main__':
    ...
    """
    test the code by running the followin commands
...
curl -H "Content-type: application/json" -d "{\"nodes\" :[\"http://localhost:5000\"]}" -X POST http://localhost:5001/nodes/add_nodes

and

curl -H "Content-type: application/json" -d "{\"nodes\" :[\"http://localhost:5001\"]}" -X POST http://localhost:5000/nodes/add_nodes
